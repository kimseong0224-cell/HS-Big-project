...
export default function HomepageConsultingInterview({ onLogout }) {
  ...
  const handleNext = () => {
    ...
    const isEmptyGroup = (keys) =>
      !keys.some((k) => String(form?.[k] || "").trim());

    // ✅ 필수 다 채운 상태(완료)에서도 Next 계속 동작
    if (label === "완료") {
      const DESIGN_KEYS = [
        "styleTone",
        "referenceSites",
      ...
      const TECH_KEYS = ["features", "integrations", "cms"];
      const CONSTRAINT_KEYS = ["constraints", "deadline", "budget", "notes"];

      if (isEmptyGroup(DESIGN_KEYS)) return scrollToSection(refDesign);
      if (isEmptyGroup(TECH_KEYS)) return scrollToSection(refTech);
      if (isEmptyGroup(CONSTRAINT_KEYS)) return scrollToSection(refConstraints);
      return;
    }
